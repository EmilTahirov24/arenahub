...
export default function TeamLoginPage() {
  const [state, formAction, pending] = useActionState(teamLogin, undefined);
  ...
  return (
    <div className="flex min-h-screen items-center justify-center px-4">
      <form action={formAction} className="w-full max-w-sm rounded-xl border border-border-subtle bg-surface p-6">
        ...
        <p className="mb-6 text-sm text-foreground-muted">Komanda panelinə giriş</p>
        ...
        <p className="mb-4 text-right text-xs">
          <Link href="/team/forgot-password" className="text-brand-via hover:underline">
            Şifrəni unutmusunuz?
          </Link>
        </p>
        ...
        <p className="mt-4 text-center text-xs text-foreground-muted">
          Hesabınız yoxdur?{" "}
          <Link href="/team/register" className="text-brand-via hover:underline">
            Qeydiyyatdan keçin
          </Link>
        </p>
      </form>
