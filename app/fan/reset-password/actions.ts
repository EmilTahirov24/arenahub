...
export async function resetFanPassword(_prevState: { error?: string } | undefined, formData: FormData) {
  ...
  const fan = await prisma.fan.findUnique({ where: { resetToken: token } });

  if (!fan || !isResetTokenValid(fan, token)) {
