...
import { requestFanPasswordReset } from "./actions";

export default function FanForgotPasswordPage() {
  const [state, formAction, pending] = useActionState(requestFanPasswordReset, undefined);
      ...
      <form action={formAction} className="w-full max-w-sm rounded-xl border border-border-subtle bg-surface p-6">
        ...
        {state?.success ? (
          <p className="mb-4 text-sm text-foreground">
            Əgər bu email ilə qeydiyyatdan keçmiş izləyici varsa, şifrə sıfırlama linki göndərildi. Zəhmət olmasa
            emailinizi yoxlayın.
          </p>
        ) : (
          <>
