...
// Provider choice, sender address and the connection itself live in
// lib/emailTransport.ts so that scripts/check-email.ts can exercise the real
// thing — this module is server-only and a script cannot import it.
...
async function send(msg: { to: string; url: string; label: string; subject: string; html: string }): Promise<EmailResult> {
  ...
  try {
    ...
    if (via === "smtp") resetSmtpTransporter();
